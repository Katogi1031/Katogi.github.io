...
// var navPos = jQuery( '#sticky_nav' ).offset().top; // グローバルメニューの位置
// var navHeight = jQuery( '#sticky_nav' ).outerHeight(); // グローバルメニューの高さ
// jQuery( window ).on( 'scroll', function() {
// 	if ( jQuery( this ).scrollTop() > navPos ) {
// 		jQuery( 'body' ).css( 'padding-top', navHeight );
// 		jQuery( '#sticky_nav' ).addClass( 'fixed' );
// 	} else {
// 		jQuery( 'body' ).css( 'padding-top', 0 );
// 		jQuery( '#sticky_nav' ).removeClass( 'fixed' );
// 	}
...
_window.on('scroll',function(){
    ...
    if(_window.scrollTop() > 70+heroBottom){
        _header.addClass('fixed');   
    }
    else{
        _header.removeClass('fixed');   
    }
    console.log("height=", heroBottom);
});
